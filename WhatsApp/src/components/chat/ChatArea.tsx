...
export const ChatArea: React.FC = () => {
  const { chats, startCall } = useWhatsApp();
  const { conversationId } = useParams<{ conversationId?: string }>();
  const navigate = useNavigate();
  const chatBottomRef = useRef<HTMLDivElement | null>(null);

  const targetId = conversationId || "chat_1";
  const activeChat = chats.find((c) => c.id === targetId) || chats[0];
    ...
    <div className="wa-chat-panel">
      ...
      <div className="chat-canvas">
        ...
        <div style={{ display: "flex", justifyContent: "center", margin: "10px 0 20px 0" }}>
          ...
          >
            ℹ️ SIMULATION ONLY: End-to-end encryption is not yet implemented. Messages are processed locally in browser simulation mode.
          </div>
        </div>
