...
export const StatusViewerModal: React.FC = () => {
  const { activeStatusGroup, setActiveStatusGroup, statusStories } = useWhatsApp();
  const { statusId } = useParams<{ statusId?: string }>();
  const navigate = useNavigate();
  const [replyText, setReplyText] = useState<string>("");

  const currentGroup = activeStatusGroup || statusStories.find((s) => s.id === statusId) || null;
      ...
      >
        ...
        <div style={{ padding: "12px 16px", backgroundColor: "rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12 }}>
            <img
              src={currentGroup.contact.avatar}
              alt={currentGroup.contact.name}
              style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover" }}
            />
            <div>
              <div style={{ fontWeight: 700, fontSize: "0.9rem", color: "white" }}>
                {currentGroup.contact.name}
              </div>
