...
import { Search, Filter, Pin, CheckCheck, Check, Clock, Image as ImageIcon, Mic } from "lucide-react";
import { useWhatsApp } from "../../context/WhatsAppContext";
import { Message } from "../../types";

export const ChatList: React.FC = () => {
  const { chats, markChatAsRead } = useWhatsApp();
  const navigate = useNavigate();
  const { conversationId } = useParams<{ conversationId?: string }>();
  const [searchQuery, setSearchQuery] = useState("");
  const [filterUnread, setFilterUnread] = useState(false);

  const renderLastMsgStatus = (lastMsg?: Message) => {
    if (lastMsg?.senderId !== "user_me") return null;
    if (lastMsg.status === "read" || lastMsg.status === "READ") {
      return (
        <span title="Read">
    ...
    if (lastMsg.status === "delivered" || lastMsg.status === "DELIVERED") {
      return (
    ...
    if (lastMsg.status === "sent" || lastMsg.status === "SENT" || lastMsg.status === "ACCEPTED") {
      return (
    ...
    <div className="wa-chat-list-view">
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {filteredChats.map((c) => {
          ...
          const isActive = c.id === conversationId || (!conversationId && c.id === "chat_1");
